...
%% @doc tries to find a db_node process
%% @spec find_db_node() -> pid()
find_db_node() ->
    gen_server:call(?MODULE, {find_process, dbs_node}, 20000).

%% @doc tries to find all db_node processes
-spec(find_all_db_nodes/0 :: () -> list()).
find_all_db_nodes() ->
    find_all_processes(db_node).
